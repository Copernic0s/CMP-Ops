...
- in the current workspace, `.env` is set to the existing `Profile 8` session, which now opens directly on `/owners`; the `Citifuel` bookmark in that profile also goes directly to `/owners`
